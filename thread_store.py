...
class ThreadStore:
    ...
    def _load(self):
        ...
        self._threads = {
            ...
            for root_id, record in raw.items()
        }

    def _save(self):
        self._path.write_text(json.dumps(self._threads, indent=2), "utf-8")
...
def build_inbox_view(messages: list[dict], thread_store: ThreadStore, *, actor: str, channel: str | None = None) -> dict:
    ...
    for message in scoped:
        ...
        if actor_lower not in tags and "all" not in tags:
            continue
        mentions.append({
            "message_id": message["id"],
            "thread_root_id": resolve_thread_root_id(message, message_index),
            "channel": message.get("channel", "general"),
            "sender": message.get("sender", ""),
            "text": message.get("text", ""),
            "time": message.get("time", ""),
            "is_broadcast": "all" in tags,
        })
